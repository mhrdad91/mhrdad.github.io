...
var initProgram = setTimeout(function(){
    ...
    var message = "RUNNING GITHUB RESUME";
    var $identityDiv = $("#identity-results");
    var $name = "NAME: MEHRDAD DOLATABADI";
    var $alias = "KNOWN ALIAS: MHRDAD";
    var $email = "EMAIL: MEHRDADDOLATABADI91@GMAIL.COM";
    var $resume = "DOWNLOAD MY CV FROM HERE";
    var $occupation  = "OCCUPATION: DATA ENGINEER | BACK-END DEVLOPER";
    var $dataEngineering  = "DATA ENGINEERING TECHNOLOGIES:";
    var $nosqlDatabases = "NOSQL DATABASES: CASSANDRA | ELASTICSEARCH | MONGODB";
    var $JSLibs = "SQL DATABASES: MYSQL | SQLITE";
    var $CSSFrameworks = "MONITORING TOOLS: GRAFANA | PROMETHEUS";
    var $CSSPre = "DATA MODEL DESIGNING";

    // var $frontEndAnimation = "FRONT END ANIMATION: CANVAS | HTML5 | SVG";
    // var $cmsDevelopment = "BACK-END DEVELOPMENT: PYTHON";
    // var $wordpress = "WORDPRESS: CUSTOM THEMING | PLUGINS";

    var $design = "BACK-END DEVELOPMENT: PYTHON";
    var $webDesign = "FRAMEWORKS: DJANGO | FLASK";
    var $branding = "LIBRARIES: PANDAS | NUMPY | SCIKIT-LEARN";
    var $designAnimation = "RESTFUL API";
    ...
    function initIdentityResults(i){
        ...
        function initEmail(i){
            $("#email").addClass("sign cursor").text($email.substring(0, i));
            if(i < $email.length){
                setTimeout(function(){
                    initEmail(i + 1);
                }, 35);
            }else{
                $("#email").removeClass("cursor");
                setTimeout(function() {
                    initResume(0);
                }, 1500);
            }
        ...
        function initResume(i){
            $("#cv").addClass("sign cursor").text($resume.substring(0, i));
            if(i < $resume.length){
                setTimeout(function(){
                    initResume(i + 1);
                }, 35);
            }else{
                $("#cv").removeClass("cursor");
                setTimeout(function() {
                    initOccupation(0);
                }, 1500);
            }
        ...
        function initFrontEnd(i){
            ...
            $('#front-end').addClass("cursor").text($dataEngineering.substring(0, i));
            if(i < $dataEngineering.length){
                setTimeout(function(){
                    initFrontEnd(i + 1);
                }, 35);
            }else{
                ...
                setTimeout(function() {
                    initNoSqlDatabases(0);
                }, 1500);
            }
        }
        function initNoSqlDatabases(i){
            $('#nosql-databases').addClass("sign cursor").text($nosqlDatabases.substring(0, i));
            if(i < $nosqlDatabases.length){
                setTimeout(function(){
                    initNoSqlDatabases(i + 1);
                }, 35);
            }else{
                $('#nosql-databases').removeClass("cursor");
                setTimeout(function() {
                    initJSLibs(0);
                }, 1500);
            }
...
*/
        function initDesign(i){
            $('#design-span').addClass('fa fa-wrench');
